...
async def block_0(tmp_dir: str) -> None:
    ...
    if results:
        ...
        check(
            "Title is not a URL",
            not str(first.get("Title", "")).startswith("http"),
            f"Title={first.get('Title')}",
        )
        check(
            "# column is an integer", isinstance(first.get("#"), int), f"got {type(first.get('#'))}"
        )
...
async def block_1(tmp_dir: str) -> tuple[str | None, list[dict]]:
    ...
    results = await search_papers("realized volatility HAR model", max_per_source=5)
    ...
    if results:
        ...
        check(
            "Title not a URL",
            not results[0]["Title"].startswith("http"),
    ...
    if paper_id:
        ...
        check(
            "abstract contains title header",
            "**" in abstract_text or len(abstract_text) > 50,
...
async def block_2(paper_id: str) -> bool:
    ...
    check("campus_trip_needed key present", "campus_trip_needed" in bm, f"keys={list(bm.keys())}")
    campus_needed = bm.get("campus_trip_needed", False)

    needed = await list_needed()
    if campus_needed:
        check(
            "paper in list_needed when campus_needed=True",
            paper_id in str(needed),
            f"needed={str(needed)[:200]}",
        )
    else:
    ...
    check(
        "with_doi_hint or without_doi_hint >= 1",
...
async def block_3(paper_id: str) -> None:
    ...
    check("paper appears in search_by_tag", paper_id in ids_by_tag, f"ids={ids_by_tag[:5]}")
...
async def block_4(paper_id: str) -> None:
    ...
    check("update_note replaces content", notes2 == "Replaced note", f"notes={notes2!r}")
...
async def block_5(paper_id: str) -> None:
    ...
    check(
        "bibtex starts with @article{",
        bibtex.strip().startswith("@article{"),
        f"starts_with={bibtex[:30]!r}",
    )

    apa = await cite(paper_id, "apa")
    check("apa is non-empty string", isinstance(apa, str) and len(apa) > 10)
    check("apa does not start with @", not apa.strip().startswith("@"), f"starts_with={apa[:20]!r}")
...
async def block_6(paper_id: str) -> None:
    ...
    if related:
        ...
        check(
            "Signal key present on all results",
            signals_present,
...
async def block_8(tmp_dir: str) -> list[str]:
    ...
    results = await search_papers("GARCH volatility forecasting", max_per_source=5, save=True)
    ...
    if bulk_ids:
        bm_result = await bulk_bookmark(
            bulk_ids, reason="UAT bulk test", tags=["uat-bulk", "to-read"]
        )
        check("bulk_bookmark returns dict", isinstance(bm_result, dict))
        check("bookmarked >= 1", bm_result.get("bookmarked", 0) >= 1, f"result={bm_result}")
        check(
            "not_found == 0",
            bm_result.get("not_found", 0) == 0,
