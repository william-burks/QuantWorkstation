...
> ```
> CURRENT schema nodes: Strategy, Run, Config, Champion, RetiredChampion, FormerChampion (QWS-0801),
...
### [CURRENT] — FormerChampion Edges (QWS-0801)
...
| `former_champions` | Cemetery view: all FormerChampion nodes with `oos_reason`, `retirement_note`, `status` (DEGRADED \| RETIRED). LLM checks before proposing any new strategy. |
...
| ~~`former_champions`~~ | QWS-0801 **(IMPLEMENTED — see CURRENT tools above)** | — |
...
**Schema note:** `oos_reason` and `retirement_note` are now live properties on `FormerChampion` /
`RetiredChampion` — implemented in QWS-0801. Use `qw query --name former_champions` to access the cemetery view.
