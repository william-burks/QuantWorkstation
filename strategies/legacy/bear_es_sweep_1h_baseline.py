"""
ES Bear Sweep Strategy — 1H baseline

Detects liquidity sweeps on 1H ES charts and trades the post-sweep reversal.
Uses identical confirmation logic to CL bear sweep but adapted for equity index futures.

Phase 1 baseline: all sessions, no wick filter, default target_r=1.25.
This establishes which sessions carry the signal on ES.
"""
...
# PARAMETERS — baseline profile (same as CL with atr_mult_stop open for grid)
DEFAULT_CONFIG = {
    'signal_window': 36,
    'atr_mult_stop': 0.5,
    'target_r': 1.25,
...
def _parse_date_bound(date_str, end_of_day=False):
    """Parse YYYY-MM-DD into US/Eastern timestamp bound."""
    if date_str is None:
        return None
...
def _apply_date_window(df, start_ts, end_ts):
    """Filter dataframe index to inclusive [start_ts, end_ts] window."""
    out = df
...
def prepare_data(config):
    ...
    try:
        ...
        raise FileNotFoundError(
            f"MES data files not found at {MES_5M} or {MES_1H}. "
            "Ensure MES_continuous_5min.parquet and MES_continuous_1H.parquet exist."
        )

    try:
        mnq5 = load(MNQ_5M)
    except FileNotFoundError:
        # MNQ used for SMT divergence; graceful fallback if not available
        mnq5 = pd.DataFrame()

    start_ts = _parse_date_bound(config.get('start_date'))
    end_ts = _parse_date_bound(config.get('end_date'), end_of_day=True)

    mes5 = _apply_date_window(mes5, start_ts, end_ts)
    mes1h = _apply_date_window(mes1h, start_ts, end_ts)
    if not mnq5.empty:
        mnq5 = _apply_date_window(mnq5, start_ts, end_ts)

    mes5['atr'] = compute_atr(mes5)
    mes1h['atr'] = compute_atr(mes1h)

    mes5['session'] = [label_session(i) for i in mes5.index]
    mes1h['session'] = [label_session(i) for i in mes1h.index]

    mes1h = swing_pivots(mes1h, n=config['swing_n'])

    mes5 = detect_fvg(mes5)
    mes5 = detect_ifvg(mes5)
...
def run_backtest(data, config):
    ...
    for _, sweep in sweeps_for_trading.iterrows():
        ...
        sw_bar = mes1h[mes1h.index == sweep_time]
        sweep_close = float(sw_bar['close'].iloc[0]) if len(sw_bar) else np.nan

        window = mes5[mes5.index > sweep_time].head(config['signal_window'])
        if len(window) < 4:
            continue

        # SMT gracefully handles missing MNQ
        if not mnq5.empty:
            mnq_win = mnq5.reindex(window.index, method='ffill').ffill()
            smt_arr = detect_smt(mes5.reindex(window.index, method='ffill').ffill(), mnq_win, direction)
        else:
        ...
        forward = mes5[mes5.index > entry_row.name].head(config['max_hold_bars'])
...
def print_backtest_report(result, loaded_rows):
    ...
    print(f"loaded_rows: mes5={loaded_rows['mes5']} mes1h={loaded_rows['mes1h']} "
          f"mnq5={loaded_rows['mnq5']}")
    ...
    if n < 20:
        ...
        print(
            ...
            f"{result['config']['target_r']}R. ES bear sweep 1H strategy shows positive expectancy — "
...
def build_arg_parser():
    ...
    parser.add_argument('--start-date', type=str, default=None, help='Start date (YYYY-MM-DD) for temporal window')
    parser.add_argument('--end-date', type=str, default=None, help='End date (YYYY-MM-DD) for temporal window')
...
def main():
    ...
    loaded_rows = {
        'mes5': len(data['mes5']),
        'mes1h': len(data['mes1h']),
        'mnq5': len(data['mnq5']),
    }

    if data['start_ts'] is not None or data['end_ts'] is not None:
        print(
            f"time_window: start={data['start_ts']} end={data['end_ts']}"
        )
