...
PRESET_CATALOG: dict[str, PresetSpec] = {
    ...
    "former_champions": PresetSpec(
        ...
        description=(
            "Cemetery view: all FormerChampion nodes ordered by degraded_at DESC. "
            "Columns: former_champion_id, strategy_id, instrument, degraded_at, "
            "oos_reason, retirement_note (null if still DEGRADED), "
            "status (DEGRADED | RETIRED). Use before proposing a strategy to check "
            "whether the idea is a reskin of a dead edge."
        ),
        params=(),
        requires_graph=True,
    ),
