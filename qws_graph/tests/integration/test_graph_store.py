...
CHAMPION_FIXTURE = REPO_ROOT / "tests" / "fixtures" / "artifacts" / "champion" / "es_bear_sweep_1h_v1.md"
...
def _count_graph() -> dict[str, int]:
    ...
    try:
        with driver.session(database="neo4j") as session:
            ...
            has_run = session.run("MATCH (:Strategy)-[r:HAS_RUN]->(:Run) RETURN count(r) AS c").single()["c"]
            uses_config = session.run("MATCH (:Run)-[r:USES_CONFIG]->(:Config) RETURN count(r) AS c").single()["c"]
            produced = session.run("MATCH (:Strategy)-[r:PRODUCED_CHAMPION]->(:Champion) RETURN count(r) AS c").single()["c"]
            pivoted = session.run("MATCH (:Champion)-[r:PIVOTED_FROM]->(:Run) RETURN count(r) AS c").single()["c"]
            return {
                "Strategy": strategy,
                "Run": run_count,
                "Config": config,
...
def test_champion_ingest_creates_produced_and_optional_pivoted_from(graph_store: GraphStore) -> None:
...
def test_infra_errors_propagate_as_store_layer_exceptions() -> None:
    ...
    broken = GraphStore(uri="bolt://127.0.0.1:1", username="neo4j", password="password", timeout_seconds=1)
