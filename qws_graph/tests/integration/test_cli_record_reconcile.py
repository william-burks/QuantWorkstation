...
from research.graph.store import EvolutionOutcome, StoreResult
...
class TestBundleHypothesisAutolink:
    """E2E tests for QWS-HF-001: bundle hypothesis_id autolink + branched-from node creation."""

    @mock.patch("research.graph.cli.GraphStore.from_env")
    @mock.patch("research.graph.cli.NeoConnector.is_available")
    def test_bundle_with_hypothesis_id_creates_tested_as_edge(
        self,
        mock_is_available,
        mock_store_factory,
        temp_repo_root: Path,
    ) -> None:
        """Bundle ingest with hypothesis_id in manifest creates TESTED_AS edge."""
        mock_is_available.return_value = True
        mock_store = mock.MagicMock()
        mock_store.persist_artifact.return_value = StoreResult(
            status="persisted",
        ...
        bundle_dir = temp_repo_root / "test_bundle"
        bundle_dir.mkdir()
    ...
    def test_bundle_dry_run_skips_hypothesis_link(
        ...
        """Dry-run bundle with hypothesis_id does NOT create TESTED_AS edge."""
        mock_is_available.return_value = True
        mock_store = mock.MagicMock()
        mock_store_factory.return_value = mock_store

        bundle_dir = temp_repo_root / "dry_bundle"
        bundle_dir.mkdir()
        csv_file = bundle_dir / "baseline_results.csv"
        csv_file.write_text(
            "instrument,timeframe,direction,logic_type,timestamp,sharpe,profit_factor,"
...
class TestHypothesisBranchedFromNodeCreation:
    """E2E tests for QWS-HF-001 (GAP-010): --hypothesis title + --branched-from."""
    ...
    def test_title_plus_branched_from_creates_node_and_edge(
        ...
        """Single call with title + --branched-from creates Hypothesis node and BRANCHED_FROM edge."""
        mock_is_available.return_value = True
        mock_store = mock.MagicMock()
        ...
        args.bundle = None
        args.oos = None
        args.hypothesis = "bear liquidity sweep tighter stop"
        args.tested_as = None
        args.branched_from = "63bcef04513b"
        args.rationale = "trial showed Sharpe 2.3 but -18% DD"
        args.status = None
        args.timeout_seconds = 3
    ...
    @mock.patch("research.graph.cli.GraphStore.from_env")
    @mock.patch("research.graph.cli.NeoConnector.is_available")
    def test_id_plus_branched_from_skips_node_creation(
        self,
        mock_is_available,
        mock_store_factory,
    ) -> None:
        """Existing 12-char hex ID + --branched-from does NOT create a new Hypothesis node."""
        mock_is_available.return_value = True
        mock_store = mock.MagicMock()
        mock_store.link_hypothesis_branched_from.return_value = True
        mock_store_factory.return_value = mock_store
        ...
        args.bundle = None
        args.oos = None
        args.hypothesis = "a0e7380e58e8"  # looks like an ID
        args.tested_as = None
        args.branched_from = "63bcef04513b"
        args.rationale = "refining the approach"
        args.status = None
        args.timeout_seconds = 3
        args.similarity_threshold = None
