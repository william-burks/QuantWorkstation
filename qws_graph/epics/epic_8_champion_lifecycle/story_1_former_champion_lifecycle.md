...
- type: cli
- cmd: `python -m qws_graph.research.graph.cli degrade --champion demo_champ_001 --reason "OOS fail — MaxDD breach"`
- expect_contains: "OK: Champion 'demo_champ_001' degraded"
- expect_exit: 0

### AC2: qw degrade without --reason exits non-zero
- type: cli
- cmd: `python -m qws_graph.research.graph.cli degrade --champion demo_champ_001 --reason ""`
- expect_contains: "ERROR: --reason must be a non-empty string"
- expect_exit: 1

### AC3: qw degrade with empty string reason exits non-zero
- type: regression
- Covered by AC2 (same code path — empty string after strip)

### AC4: qw degrade nonexistent champion exits non-zero
- type: cli
- cmd: `python -m qws_graph.research.graph.cli degrade --champion nonexistent_id_xyz --reason "test"`
- expect_contains: "ERROR"
- expect_exit: 1

### AC5: qw retire with note creates RetiredChampion and RETIRED_TO edge
- type: cli
- cmd: `python -m qws_graph.research.graph.cli retire --former-champion <fc_id> --note "Logic dead-ended"`
- expect_contains: "OK: FormerChampion"
- expect_exit: 0

### AC6: qw retire stores retirement_note on RetiredChampion
- type: cypher
- cmd: `MATCH (rc:RetiredChampion) WHERE rc.retirement_note IS NOT NULL RETURN count(rc) > 0 AS has_note`
- expect_contains: "true"

### AC7: qw retire without --note succeeds
- type: cli
- cmd: `python -m qws_graph.research.graph.cli retire --former-champion <fc_id>`
- expect_exit: 0

### AC8: qw query --name former_champions returns cemetery view
- type: cli
- cmd: `python -m qws_graph.research.graph.cli query --name former_champions --json`
