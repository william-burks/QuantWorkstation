...
### Second promotion path — CHAMPION_INGEST_QUERY (cypher.py)

The bundle ingest path also creates WAS_CHAMPION via CHAMPION_INGEST_QUERY in cypher.py.
Add SUPERSEDED_BY in the same FOREACH block there as well, matching the same pattern.
Both paths must create the edge atomically so lineage is complete regardless of how
promotion occurs.
...
- `qws_graph/research/graph/cypher.py` — add SUPERSEDED_BY alongside WAS_CHAMPION in CHAMPION_INGEST_QUERY FOREACH block
...
- `qws_graph/research/graph/cypher.py` — add SUPERSEDED_BY alongside WAS_CHAMPION in CHAMPION_INGEST_QUERY
