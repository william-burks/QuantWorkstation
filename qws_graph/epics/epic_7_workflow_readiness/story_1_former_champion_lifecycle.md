...
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli degrade --champion demo_champ_001 --reason "OOS fail CPI spike"`
- expect_contains: "OK"
- expect_exit: 0

### AC2: qw degrade without --reason exits non-zero
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli degrade --champion demo_champ_001`
- expect_contains: "error"
- expect_exit: 2

### AC3: qw degrade with empty reason exits non-zero
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli degrade --champion demo_champ_001 --reason ""`
- expect_contains: "ERROR"
- expect_exit: 1

### AC4: qw degrade with non-existent champion exits non-zero
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli degrade --champion nonexistent_xyz --reason "test"`
- expect_contains: "not found"
- expect_exit: 1

### AC5/6: qw retire creates RetiredChampion + stores retirement_note
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli retire --former-champion demo_fc_001 --note "No pivot hypothesis"`
- expect_contains: "OK"
- expect_exit: 0

### AC7: qw retire without --note succeeds
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli retire --former-champion demo_fc_001`
- expect_contains: "OK"
- expect_exit: 0

### AC8: former_champions preset returns cemetery view
- type: cli
- cmd: `source .venv/bin/activate && python -m research.graph.cli query --name former_champions`
- expect_contains: "demo_fc_001"
- expect_exit: 0

### AC9: Champion node readable after demotion
- type: cypher
- cmd: `MATCH (ch:Champion {champion_id: 'demo_retired_champ_001'}) RETURN ch.champion_id`
- expect_contains: "demo_retired_champ_001"
