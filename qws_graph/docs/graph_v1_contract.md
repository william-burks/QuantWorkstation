...
- `strategy_lineage`
- `pending_offline`
- `former_champions` — cemetery view (QWS-0801)

---

### `qw degrade` (QWS-0801)
Purpose:
- Demote a Champion to FormerChampion with a mandatory cause-of-death string.

Usage:
```text
qw degrade --champion <champion_id> --reason "<oos_reason>" [--timeout-seconds <int>]
```

Options:
- `--champion <id>`: champion_id of the Champion node to demote (required)
- `--reason <str>`: mandatory cause-of-death / OOS failure reason (must be non-empty)
- `--timeout-seconds <int>`: Neo4j timeout (default 3)

Exit codes:
- `0`: Champion found, FormerChampion created, DEGRADED_TO edge written
- `1`: empty reason OR Champion not found in graph
- `2`: Neo4j unavailable or write failure

Effect:
- Creates `:FormerChampion` node with `former_champion_id = hash12(champion_id, degraded_at_iso)`
- Creates `(Champion)-[:DEGRADED_TO]->(FormerChampion)` edge
- Champion node is NOT deleted — remains queryable

---

### `qw retire` (QWS-0801)
Purpose:
- Retire a FormerChampion to a RetiredChampion with an optional note.

Usage:
```text
qw retire --former-champion <former_champion_id> [--note "<note>"] [--timeout-seconds <int>]
```

Options:
- `--former-champion <id>`: former_champion_id of the FormerChampion node (required)
- `--note <str>`: optional free-text retirement note
- `--timeout-seconds <int>`: Neo4j timeout (default 3)

Exit codes:
- `0`: FormerChampion found, RetiredChampion created, RETIRED_TO edge written
- `1`: FormerChampion not found in graph
- `2`: Neo4j unavailable or write failure

Effect:
- Creates `:RetiredChampion` node with `oos_reason` copied from FormerChampion
- Creates `(FormerChampion)-[:RETIRED_TO]->(RetiredChampion)` edge
- Sets `retirement_note` on RetiredChampion (empty string if not provided)
