...
for msn in ["ZWHDPUS", "ZWCDPUS"]:
    oldest = get("/total-energy/data/", {
        "facets[msn][]": msn, "length": "1",
        "sort[0][column]": "period", "sort[0][direction]": "asc",
    })
    newest = get("/total-energy/data/", {
        "facets[msn][]": msn, "length": "1",
    ...
    print(f"{msn}: {total} rows | {first.get('period')} → {last.get('period')} | unit={first.get('unit')} | value={first.get('value')}")
