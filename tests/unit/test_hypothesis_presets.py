...
from research.graph.query_presets import PRESET_CATALOG, run_preset
...
class TestFindingsFlag:
    ...
    def test_findings_update_calls_store_method(self) -> None:
        ...
        with (
            patch("research.graph.cli.NeoConnector", return_value=mock_connector),
            patch("research.graph.cli.GraphStore") as mock_gs_class,
        ):
    ...
    def test_findings_neo4j_unavailable_returns_exit_2(self) -> None:
        ...
        with patch("research.graph.cli.NeoConnector", return_value=mock_connector):
            exit_code = _cmd_hypothesis(args)
